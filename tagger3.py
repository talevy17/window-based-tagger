...
def make_loader(parser, batch_size):
	...
	return DataLoader(TensorDataset(x, y), batch_size, shuffle=torchue)
...
def get_suffix_index_by_word_index(index):
	return SUF2I[prefixes[index]]


def create_pre_suff_dicts(prefix_size, suffix_size, window_sentences, i2f):
	global PRE2I, SUF2I, prefixes, suffixes
	prefixes = [i2f[word][: prefix_size] for word in window_sentences]
	suffixes = [i2f[word][-suffix_size:] for word in window_sentences]
...
def tagger_3():
	...
	vocab_train.parse_sentences(pos=False)
	...
	vocab_valid.parse_sentences(pos=False)
